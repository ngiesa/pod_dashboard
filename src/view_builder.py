...
def build_filtered_master_table(ddm, filter_condition={}):
    ...
    if len(df_master) == 0:
        ...
        logger.info("no data for master table according to filters")
    ...
    return dash_table.DataTable(
        ...
        columns=[
            ...
            if i not in ["COPID", "BEGANORG", "ENDANORG", "BEGAWORG", "ENDAWORG"]
        ],
        ...
        style_data_conditional=[
            {
                "if": {"filter_query": "{RISK} >= 0.8", "column_id": "RISK"},
            ...
            {
                "if": {"filter_query": "{RISK} <= 0.4", "column_id": "RISK"},
...
def build_details_main_view(ddm, index):
    ...
    df_plot = (
        df_vitals.groupby(["c_time", "var_name"])["c_value"]
        .mean()
        .reset_index()
        .pivot(columns="var_name", index="c_time", values="c_value")
        .reset_index()
    )

    if len(df_plot) == 0:
        logger.info("vitals for patient not found")
        return html.H1("No charts to display", style={"padding": "40px"})

    plot_cols = list(df_plot.columns)
    logger.info(plot_cols)
    ...
    for fig in [fig_pulse, fig_blood_pressure, fig_spo2, fig_rr]:
        fig.update_layout(
            ...
            height=340,
            legend=dict(orientation="h", bgcolor="white", x=0.60),
            legend_title=None,
        )

        fig.update_traces(
            marker=dict(size=8, line=dict(width=1)), selector=dict(mode="markers")
        )
    ...
    return html.Div(
        ...
        children=[
            html.Div(
                children=[
                    dcc.Graph(
                        className="vital-graphs",
                        figure=fig_blood_pressure,
                        style=style,
