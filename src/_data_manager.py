...
class DashDataManager:
    ...
    def load_data(self):
        """ loads data """
        self.data_op_master = pd.read_csv(
            "./conn/hdl/data/data_op_master.csv", index_col=0, low_memory=False
        )
        self.df_premed = pd.read_csv(
            "./conn/hdl/data/data_premed.csv", index_col=0, low_memory=False
        )
        self.df_vital = pd.read_csv(
            "./conn/hdl/data/data_vital.csv", index_col=0, low_memory=False
        )

    def get_last_updated(self):
        """ gets last entry in op master table """
        last_update = (
            self.reset_filter().sort_values(["DATE", "BEGAN"], ascending=False).iloc[0]
        )
    ...
    def prepare_items(self):
        ...
        max_version = (
            self.df_premed.groupby(["c_op_id", "c_var_id"])["c_version"]
            .max()
            .reset_index()
            .drop_duplicates()
        )
        df_premed_main = self.df_premed.merge(
            max_version, on=["c_op_id", "c_var_id", "c_version"], how="inner"
        )
        df_premed_main = (
            df_premed_main.drop(columns=["c_date_time_to", "c_id"])
            .drop_duplicates()
            .merge(
                self.var_df[self.var_df.var_name.isin(self.prem_items)],
                on="c_var_id",
                suffixes=["_", ""],
            )
            .dropna()
            .pivot(index=["c_op_id"], columns="var_name", values="c_val")
            .reset_index()
        )
    ...
    def get_display_main_table(self, df_main):
        ...
        df_main = df_main[
            ...
            + self.TIME_COLS
            + self.display_main_items
            + [c + "_ORG" for c in self.TIME_COLS]
        ]
        columns = [
            x.replace("prem_", "").replace("_", "").upper() for x in df_main.columns
        ]
        ...
        df_main["RISK"] = PRED
        df_main["CONFID"] = [
            "({},{})".format(str(round(p - 0.02, 2)), str(round(p + 0.02, 2)))
            for p in PRED
        ]
        self.display_master_table = df_main.sort_values(
            ["DATE", "BEGAN"], ascending=False
        )
        return self.display_master_table

    def filter_main_table(
        self,
        filter_map={
            "item1": {"variable": "prem_sex", "value": "male", "condition": "equal"}
        },
    ):
        ...
        if len(list(filter_map.keys())) != 0:
            ...
            self.all_filter_conditions[key] = filter_map[
                key
            ]  # TODO STORE ALL FILTER CONDITIONS
    ...
    def reset_filter(self):
        ...
        df_display = self.filter_main_table(filter_map={}).sort_values(
            ["DATE", "BEGAN"], ascending=False
        )
        return df_display
    ...
    def resample_display_time(self, df):
        ...
        return df.assign(
            c_time=[pd.to_datetime(t[0:16]) for t in df.c_date_time_to]
    ...
    def get_vitals(self, c_root="", c_begin="", c_end=""):
        ...
        df_vital_display = df_vital[
            (df_vital.c_root_id == c_root)
            & (df_vital.c_date_time_to >= c_begin)
            & (df_vital.c_date_time_to <= c_end)
        ].merge(
            self.var_df[self.var_df.var_name.isin(self.vitals_items)], on="c_var_id"
        )
        logger.info("logger.info available variable names")
        logger.info(df_vital_display.var_name.drop_duplicates())
        # get suffix indicators
        df_vital_display = df_vital_display.assign(
            var_name=df_vital_display.var_name + df_vital_display.c_suffix.fillna("")
        )
    ...
    def get_dropdown_items(self, pream_col):
        logger.info("GET DROPDOWN ITEMS")  # TODO Implement conditional filtering
    ...
    def get_date_items(self):
        ...
        return (
            pd.to_datetime(self.data_op_master["BEGAN"].min()),
